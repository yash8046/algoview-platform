...
export default function CryptoTradeHistory() {
  ...
  const handleExport = async () => {
    try {
      ...
      const uri = await exportTradesToCSV(mapped);
      setSaved(true);
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {trades.slice(0, 20).map(trade => {
                const pair = CRYPTO_PAIRS.find(p => p.symbol === trade.pair);
                return (
                  <tr key={trade.id} className="border-b border-border">
                    <td className="px-3 py-2 text-muted-foreground font-mono">
                      {new Date(trade.timestamp).toLocaleTimeString()}
                    </td>
                    <td className="px-3 py-2 font-mono font-semibold text-foreground">{pair?.label || trade.pair}</td>
                    <td className="px-3 py-2">
                      <span className={`font-mono font-semibold uppercase ${trade.side === 'buy' ? 'text-gain' : 'text-loss'}`}>
                        {trade.side === 'buy' ? 'LONG' : 'SHORT'}
                      </span>
                    </td>
                    <td className="px-3 py-2 text-right font-mono text-foreground">{formatINR(trade.price)}</td>
                    <td className="px-3 py-2 text-right font-mono text-muted-foreground">{trade.quantity}</td>
                    <td className={`px-3 py-2 text-right font-mono font-medium ${
                      trade.pnl !== undefined ? (trade.pnl >= 0 ? 'text-gain' : 'text-loss') : 'text-muted-foreground'
                    }`}>
                      {trade.pnl !== undefined ? `${trade.pnl >= 0 ? '+' : ''}${formatINR(Math.abs(trade.pnl))}` : '—'}
                    </td>
                  </tr>
                );
              })}
