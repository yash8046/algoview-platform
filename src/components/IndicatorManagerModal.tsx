...
export default function IndicatorManagerModal({ open, onClose, indicators, onToggle, onRemove }: Props) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" onClick={onClose}>
      ...
      {/* Modal */}
      <div
        className="relative w-full max-w-md max-h-[80vh] flex flex-col rounded-2xl border border-border/50 bg-card/95 backdrop-blur-xl shadow-2xl animate-in zoom-in-95 fade-in duration-300"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/30">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">{indicators.length} active</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-accent/50 text-muted-foreground hover:text-foreground transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="px-5 py-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 bg-secondary/50 border border-border/30 rounded-xl text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary/50 transition-all"
            />
          </div>
        </div>
