...
import {
  ...
  TrendingUp, TrendingDown, BarChart3, AlertTriangle, ArrowUpDown, Copy, Film,
} from 'lucide-react';
...
function MetricCard({ label, value, sub, positive }: { label: string; value: string; sub?: string; positive?: boolean }) {
  return (
    <div className="bg-card rounded-lg border border-border p-2">
      ...
      <div className={`font-mono text-[11px] font-bold ${positive === undefined ? 'text-foreground' : positive ? 'text-gain' : 'text-loss'}`}>{value}</div>
...
function ConditionRow({ condition, onChange, onRemove }: {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1 p-1.5 bg-secondary/50 rounded-md border border-border/50">
      <select value={condition.left} onChange={e => onChange({ ...condition, left: e.target.value as ConditionSource })}
        className="bg-background text-foreground text-[9px] font-mono px-1 py-0.5 rounded border border-border min-w-[60px]">
      ...
      <select value={condition.operator} onChange={e => onChange({ ...condition, operator: e.target.value as ConditionOperator })}
        className="bg-background text-foreground text-[9px] font-mono px-1 py-0.5 rounded border border-border">
      ...
      {condition.right === 'value' && (
        <input type="number" value={condition.rightValue ?? 0}
          ...
          className="bg-background text-foreground text-[9px] font-mono px-1 py-0.5 rounded border border-border w-14" />
      )}
      <button onClick={onRemove} className="p-0.5 text-loss/60 hover:text-loss"><Trash2 className="w-3 h-3" /></button>
    </div>
  );
}
...
function IndicatorBadge({ config, onRemove }: { config: IndicatorConfig; onRemove: () => void }) {
  ...
  return (
    <div className="flex items-center gap-1 px-1.5 py-0.5 bg-primary/10 text-primary rounded text-[9px] font-mono border border-primary/20">
      <Layers className="w-2.5 h-2.5" />
      <span>{config.type}({params})</span>
      <button onClick={onRemove} className="ml-0.5 text-primary/50 hover:text-loss"><Trash2 className="w-2.5 h-2.5" /></button>
    </div>
  );
}
...
export default function StrategyBuilderPage() {
  ...
  const downloadCSV = async () => {
    ...
    try {
      if (Capacitor.isNativePlatform()) {
        ...
        toast.success('CSV saved to Documents folder', { description: filename });
      } else {
  ...
  const strategyPanel = (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      {/* Templates */}
      <div>
        <label className="text-[9px] text-muted-foreground block mb-1">Templates</label>
        <div className="flex flex-wrap gap-1 max-h-20 overflow-y-auto overscroll-contain scrollbar-thin">
          {STRATEGY_TEMPLATES.map(t => (
            <button key={t.name} onClick={() => loadTemplate(t)}
              className={`px-1.5 py-0.5 text-[9px] rounded min-h-[26px] active:scale-95 ${strategy.name === t.name ? 'bg-primary text-primary-foreground font-semibold' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}>
              {t.name}
            </button>
          ))}
        </div>
            ...
            {savedStrategies.map(s => (
              <div key={s.name} className="flex items-center gap-0.5">
                <button onClick={() => loadTemplate(s)}
                  className="px-1.5 py-0.5 text-[9px] rounded bg-accent text-foreground hover:bg-accent/80">
                  {s.name}
                </button>
      ...
      <div className="space-y-1.5">
        <div>
          <label className="text-[9px] text-muted-foreground block mb-0.5">Market</label>
          <div className="flex gap-1">
            {(['crypto', 'stocks'] as const).map(m => (
              <button key={m} onClick={() => { updateStrategy({ market: m }); setSymbol(m === 'crypto' ? 'BTCUSDT' : 'NIFTY 50'); }}
                className={`flex-1 px-2 py-1 text-[9px] rounded capitalize min-h-[32px] active:scale-95 ${strategy.market === m ? 'bg-primary text-primary-foreground' : 'bg-secondary text-secondary-foreground'}`}>
                {m}
              </button>
            ))}
          </div>
        ...
        <div>
          <label className="text-[9px] text-muted-foreground block mb-0.5">Asset</label>
          {strategy.market === 'crypto' ? (
            <select value={symbol} onChange={e => setSymbol(e.target.value)}
              className="w-full bg-secondary text-foreground text-[9px] font-mono px-2 py-1.5 rounded border border-border min-h-[32px]">
              {CRYPTO_PAIRS.map(p => <option key={p.symbol} value={p.symbol}>{p.label}</option>)}
            </select>
          ) : (
            <div className="relative">
              <div className="flex items-center gap-1 bg-secondary rounded border border-border px-2 py-1">
                ...
                <input type="text"
                  ...
                  className="bg-transparent text-[9px] font-mono text-foreground outline-none w-full" />
              </div>
              {stockSearch !== null && (
                <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-card border border-border rounded-md shadow-lg max-h-40 overflow-y-auto scrollbar-thin">
                  {filteredStocks.map(s => (
                    <button key={s.symbol} onMouseDown={e => e.preventDefault()}
                      onClick={() => { setSymbol(s.symbol); setStockSearch(null); }}
                      className={`w-full text-left px-2 py-1 text-[9px] hover:bg-accent ${symbol === s.symbol ? 'bg-accent' : ''}`}>
        ...
        <div>
          <label className="text-[9px] text-muted-foreground block mb-0.5">Timeframe</label>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          <button onClick={() => setAddIndicatorOpen(!addIndicatorOpen)} className="text-[9px] text-primary hover:text-primary/80 flex items-center gap-0.5">
        ...
        {addIndicatorOpen && (
          <div className="mt-1.5 p-2 bg-secondary/50 rounded-md border border-border space-y-1.5">
            <select value={newIndType} onChange={e => {
              const t = e.target.value as IndicatorType;
              setNewIndType(t);
              setNewIndParams({ ...DEFAULT_INDICATOR_PARAMS[t] });
            }} className="w-full bg-background text-foreground text-[9px] font-mono px-2 py-1 rounded border border-border">
            ...
            <div className="flex flex-wrap gap-1.5">
              {Object.entries(newIndParams).map(([key, val]) => (
                <div key={key} className="flex items-center gap-0.5">
                  <label className="text-[8px] text-muted-foreground">{key}</label>
                  <input type="number" value={val}
                    onChange={e => setNewIndParams({ ...newIndParams, [key]: Number(e.target.value) })}
                    className="bg-background text-foreground text-[9px] font-mono px-1 py-0.5 rounded border border-border w-12" />
                </div>
              ))}
            </div>
            <button onClick={addIndicator} className="w-full px-2 py-1.5 text-[9px] bg-primary text-primary-foreground rounded font-semibold min-h-[32px]">
              Add {newIndType}
            </button>
          </div>
        )}
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <label className="text-[9px] text-muted-foreground font-semibold flex items-center gap-1">
            <TrendingUp className="w-3 h-3 text-gain" /> Entry Rules
          </label>
          <button onClick={() => addCondition('entry')} className="text-[9px] text-gain hover:text-gain/80 flex items-center gap-0.5">
            <Plus className="w-3 h-3" /> Add
          </button>
        </div>
        <div className="space-y-1">
          ...
          {strategy.entryConditions.length === 0 && (
            <p className="text-[8px] text-muted-foreground/50 italic">No entry conditions</p>
          )}
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <label className="text-[9px] text-muted-foreground font-semibold flex items-center gap-1">
            <TrendingDown className="w-3 h-3 text-loss" /> Exit Rules
          </label>
          <button onClick={() => addCondition('exit')} className="text-[9px] text-loss hover:text-loss/80 flex items-center gap-0.5">
            <Plus className="w-3 h-3" /> Add
          </button>
        </div>
        <div className="space-y-1">
          ...
          {strategy.exitConditions.length === 0 && (
            <p className="text-[8px] text-muted-foreground/50 italic">No exit conditions</p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <div>
            ...
            <select value={strategy.stopLoss.type} onChange={e => updateStrategy({ stopLoss: { ...strategy.stopLoss, type: e.target.value as any } })}
              className="w-full bg-secondary text-foreground text-[9px] font-mono px-1 py-0.5 rounded border border-border mb-0.5">
            ...
            <input type="number" step={0.5} value={strategy.stopLoss.value}
              ...
              className="w-full bg-secondary text-foreground text-[9px] font-mono px-1 py-0.5 rounded border border-border" />
          </div>
          <div>
        ...
        <div className="grid grid-cols-3 gap-1.5">
          ...
          <div>
            <label className="text-[8px] text-muted-foreground block mb-0.5">Size %</label>
            <input type="number" step={1} value={strategy.positionSizePct}
              onChange={e => updateStrategy({ positionSizePct: Number(e.target.value) })}
              className="w-full bg-secondary text-foreground text-[9px] font-mono px-1 py-0.5 rounded border border-border" />
          </div>
          <div>
            <label className="text-[8px] text-muted-foreground block mb-0.5">Fee %</label>
      ...
      <div className="flex gap-1.5 pt-2 border-t border-border">
        <button onClick={() => gateStrategy(runTest)} disabled={running || strategy.entryConditions.length === 0}
          ...
          {running ? 'Running...' : '▶ Watch Ad & Run Backtest'}
        </button>
        <button onClick={saveStrategy} className="px-2.5 py-2 text-[10px] rounded bg-secondary text-foreground hover:bg-accent border border-border min-h-[44px]" title="Save Strategy">
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        {isMobile && (
          <button 
            ...
            className="flex items-center gap-2 px-3 py-2 bg-card border-b border-border active:bg-accent min-h-[44px]"
          >
            <Settings className="w-4 h-4 text-primary" />
            <span className="text-xs font-semibold text-foreground flex-1 text-left">{strategy.name}</span>
            <span className="text-[10px] text-muted-foreground">{symbol} • {interval}</span>
            <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform ${sidebarOpen ? 'rotate-180' : ''}`} />
          </button>
        )}

        {/* Sidebar / Bottom sheet */}
        {sidebarOpen && (
          <div className={`${isMobile ? 'max-h-[60vh] border-b border-border' : 'w-72 lg:w-80 border-r border-border'} bg-card flex-shrink-0 overflow-hidden`}>
            {strategyPanel}
          </div>
        )}
        ...
        {!isMobile && !sidebarOpen && (
          <button onClick={() => setSidebarOpen(true)} className="w-8 flex-shrink-0 flex items-center justify-center bg-card border-r border-border hover:bg-accent">
        ...
        <div className="flex-1 flex flex-col overflow-auto pb-20 md:pb-0">
          ...
          {!isMobile && (
            <div className="flex items-center gap-2 px-3 py-1.5 border-b border-border bg-card">
              <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-1 rounded hover:bg-accent text-muted-foreground">
          ...
          {error && (
            <div className="flex items-center gap-2 p-2 m-2 rounded-lg bg-loss/10 border border-loss/20">
          ...
          {result ? (
            <div className="p-2 space-y-2">
              {/* Metrics — 2x2 on mobile, 4-col on tablet, 8-col on desktop */}
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-1">
                <MetricCard label="Return" value={`${result.totalReturnPct >= 0 ? '+' : ''}${result.totalReturnPct.toFixed(2)}%`}
                ...
                <MetricCard label="Max DD" value={`-${result.maxDrawdownPct.toFixed(2)}%`} positive={result.maxDrawdownPct < 15} />
                ...
                <MetricCard label="Trades" value={result.totalTrades.toString()} />
              ...
              <div className="relative">
                <LWChart
                  ...
                  label={`Equity — ${result.totalTrades} trades`}
              ...
              {/* Trade Log */}
              <div className="bg-card rounded-lg border border-border overflow-hidden">
                <div className="px-2 py-1.5 bg-panel-header border-b border-border flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <BarChart3 className="w-3 h-3 text-primary" />
                    <h3 className="text-[10px] font-semibold text-foreground">Trade Log</h3>
                    <span className="text-[9px] text-muted-foreground">{result.trades.length} trades</span>
                  </div>
                  <button onClick={downloadCSV} className="flex items-center gap-1 text-[9px] text-primary hover:text-primary/80 px-2 py-1 rounded bg-primary/10 min-h-[28px]">
                  ...
                  <table className="w-full text-[9px]">
                    <thead className="sticky top-0 bg-card">
                      <tr className="text-muted-foreground border-b border-border">
                        ...
                        <th className="text-right py-1 px-1.5 hidden sm:table-cell">Qty</th>
                        <th className="text-right py-1 px-1.5">P&L</th>
                        <th className="text-right py-1 px-1.5">%</th>
                        <th className="text-right py-1 px-1.5 hidden md:table-cell">Bars</th>
                        <th className="text-right py-1 px-1.5 hidden lg:table-cell">Reason</th>
                      </tr>
                    </thead>
                    <tbody>
                      {result.trades.map((t, i) => (
                        <tr key={i} className="border-b border-border/50 hover:bg-accent/30">
                          ...
                          <td className="py-1 px-1.5 text-right font-mono hidden sm:table-cell">{t.quantity}</td>
                          <td className={`py-1 px-1.5 text-right font-mono font-semibold ${t.pnl >= 0 ? 'text-gain' : 'text-loss'}`}>
                            {t.pnl >= 0 ? '+' : ''}{t.pnl.toFixed(0)}
                          </td>
                          <td className={`py-1 px-1.5 text-right font-mono ${t.pnlPct >= 0 ? 'text-gain' : 'text-loss'}`}>
                            {t.pnlPct >= 0 ? '+' : ''}{t.pnlPct.toFixed(1)}%
                          </td>
                          <td className="py-1 px-1.5 text-right font-mono hidden md:table-cell text-muted-foreground">{t.duration}</td>
                          <td className="py-1 px-1.5 text-right hidden lg:table-cell">
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-3 text-muted-foreground p-4">
              ...
              <div className="text-center">
                <p className="text-xs">Configure your strategy</p>
                <p className="text-[9px] text-muted-foreground/60 mt-1">Define indicators, entry/exit rules, then run backtest</p>
              </div>
              {!sidebarOpen && (
                <button onClick={() => setSidebarOpen(true)} className="flex items-center gap-1.5 px-4 py-2 text-xs bg-primary text-primary-foreground rounded hover:opacity-90 min-h-[44px]">
